...
def ridge_plot_pdf_with_gmm(All_tv, Hist_counts, Hist_edges, indices, times_to_plot, n_individuals, criterion="bic", sep_threshold=1.5, scale=0.4):
    ...
    for t_idx in times_to_plot:
        ...
        # ---- compute separation if 2 components ----
        if n_comp == 2:
            sep = abs(means[0] - means[1]) / np.sqrt(stds[0]**2 + stds[1]**2)
        else:
            sep = 0

        # ---- plotting morph summaries ----
        def plot_morph(mean, std, weight):
            # line width scaled by frequency
            lw = 1 + 6 * weight

            # mean line
            ax.plot([time_val, time_val],
                    [mean - std, mean + std],
                    color='black', linewidth=lw)

            # envelope lines (left/right)
            ax.plot([time_val - 0.02, time_val - 0.02],
                    [mean - std, mean + std],
        ...
        # ---- case handling ----
        if n_comp == 1:
            plot_morph(means[0], stds[0], weights[0])

        elif n_comp == 2 and sep > sep_threshold:
            for i in range(2):
                plot_morph(means[i], stds[i], weights[i])
...
def ridge_plot_pdf_with_gmm(All_tv, Hist_counts, Hist_edges, indices, times_to_plot, n_individuals, criterion="bic", sep_threshold=1.5, scale=0.4):
    ...
    for t_idx in times_to_plot:
        ...
        # ---- plotting function ----
        def plot_morph(mean, std, weight):
            lw = 1 + 6 * weight  # thickness = frequency

            # mean line (horizontal)
            ax.plot([time_val - scale*0.5, time_val + scale*0.5],
                    [mean, mean],
                    color='black', linewidth=lw)

            # envelope lines (μ ± σ)
            ax.plot([time_val - scale*0.4, time_val + scale*0.4],
                    [mean - std, mean - std],
                    color='black', linewidth=1)
...
def ridge_plot_with_morphs(All_tv, Hist_counts, Hist_edges, indices, times_to_plot, n_individuals, hump_left, hump_right, hump_weight_left, hump_weight_right, hump_std_left, hump_std_right, mean_trait_values, stdl, speed_total, speed_left, speed_right, scale=0.4):
    ...
    for k in times_to_plot:
        ...
        # ---- plot ridge ----
        ax.fill_betweenx(centers, time_val, time_val + pdf_scaled,
                         color='gray', alpha=0.6)

        # ---- helper: get ridge width at given y ----
        def ridge_width_at(y):
            idx = np.argmin(np.abs(centers - y))
            return pdf_scaled[idx]

        # ---- ONE MORPH (fallback) ----
        if np.isnan(hump_left[k]) or np.isnan(hump_right[k]):

            mean = mean_trait_values[k]
            std = stdl[k]
            width = ridge_width_at(mean)

            ax.text(time_val + scale, mean, f"μ={mean:.2f}\nσ={std:.2f}\nv={speed_total[k]:.2f}", fontsize=8)

            # mean line
            ax.plot([time_val, time_val + width],
                    [mean, mean],
        ...
        # ---- TWO MORPHS ----
            ...
            morphs = [
                (hump_left[k], hump_std_left[k], hump_weight_left[k], 'blue'),
                (hump_right[k], hump_std_right[k], hump_weight_right[k], 'red')
            ]

            for mean, std, weight, color in morphs:

                width = ridge_width_at(mean)

                lw = 1 + 6 * weight  # frequency → thickness
...
def ridge_plot_pdf_with_gmm(All_tv, Main_3D, Hist_counts, Hist_edges, indices, times_to_plot, n_individuals, criterion="bic", sep_threshold=1.5, scale=0.4):
    ...
    for t_idx in times_to_plot:
        ...
        widths = np.diff(edges)
        shift = Main_3D[0, k]
        centers = 0.5*(edges[:-1] + edges[1:]) + shift
        ...
        # ---- fit GMM ----
        trait_values = (All_tv[k] + Main_3D[0, k]).reshape(-1,1)
        gmm, n_comp = fit_best_gmm(trait_values, criterion)
...
def ridge_plot_pdf_with_gmm(All_tv, Main_3D, Hist_counts, Hist_edges, indices, times_to_plot, n_individuals, criterion="bic", sep_threshold=1.5, scale=0.4):
    ...
    fig_width = max(10, 0.6 * n_times)   # grows with timepoints
    fig_height = 8
    ...
    for t_idx in times_to_plot:
        ...
        time_val = indices[k] / n_individuals
        # time_positions = np.linspace(0, 1, n_times)
        # time_val = time_positions[times_to_plot.index(t_idx)]
        ...
        selected_components = []
        if n_comp == 1:
            selected_components.append((means[0], stds[0], weights[0]))
        elif n_comp == 2 and sep > sep_threshold:
            for i in range(2):
                selected_components.append((means[i], stds[i], weights[i]))
        elif n_comp == 2:
            i = np.argmax(weights)
            selected_components.append((means[i], stds[i], weights[i]))
        ...
        for mean, std, weight in selected_components:
            plot_morph(mean, std, weight)

            all_upper.append(mean + 3*std)
            all_lower.append(mean - 3*std)
