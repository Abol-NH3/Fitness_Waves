...
# # save_dir=REPORTS_DIR / f"Hump_({nlist[0]},{nlist[-1]})_b1({b1list[0]},{b1list[-1]})_b2({b2list[0]},{b2list[-1]})_d1({d1list[0]},{d1list[-1]})_d2({d2list[0]},{d2list[-1]})_skip({skip})_tmax({tmax})_tlag({t_lag})_nansa({len(nansa_list)})"
...
def simulate_system(pR0, vL0, vR0, delta, g, bpd, dt=1e-2):
    ...
    vdotmax_idx = np.argmax(Vdot_vals)
    vdotmin_idx = np.argmin(Vdot_vals)

    VdotMax = Vdot_vals[vdotmax_idx]
    VdotMin = Vdot_vals[vdotmin_idx]

    tPeak = t_vals[vdotmax_idx]
    tMin = t_vals[vdotmin_idx]
...
save_dir=f"/flash/DieckmannU/Abolfazl/detection_PR({pR_list[0]},{pR_list[-1]})_bpd({bpd_list[0]},{bpd_list[-1]})_delta({delta_list[0]},{delta_list[-1]})_g({g_list[0]},{g_list[-1]})_VR({vR_list[0]},{vR_list[-1]})"
os.makedirs(save_dir, exist_ok=True)
run_parameter_sweep(pR_list, vL_list, vR_list, delta_list, g_list, bpd_list, save_dir, n_jobs=128)
...
def Metadata_Quad_Sim_V3(nlist, b1list, b2list, d1list, d2list, nansa_list, tmax, skip, t_lag, save_dir, nbins, n_jobs=6):
    ...
    def process_one_combo(i):
        ...
        init_traits = np.zeros(n_individuals)
        ...
        hump_weight_left = np.full(n_out, np.nan)
        hump_weight_right = np.full(n_out, np.nan)
        std_left = np.full(n_out, np.nan)
        std_right = np.full(n_out, np.nan)
        ...
        for k in range(n_out):
            ...
            if gmm2.bic(tv) < gmm1.bic(tv):
                ...
                if sep > 1.5:
                    ...
                    std_left[k] = stds[order[0]]
                    std_right[k] = stds[order[1]]
...
nlist = np.array([20000])
b1list = np.array([0.6])   
d1list = np.array([0.4]) 
b2list = np.array([0])
d2list = np.array([0])
nansa_list = np.arange(32)
skip = 0.01
tmax = 300 
nbins = 128
t_lag  = 200

save_dir=f"/flash/DieckmannU/Abolfazl/Hump_({nlist[0]},{nlist[-1]})_b1({b1list[0]},{b1list[-1]})_b2({b2list[0]},{b2list[-1]})_d1({d1list[0]},{d1list[-1]})_d2({d2list[0]},{d2list[-1]})_skip({skip})_tmax({tmax})_tlag({t_lag})_nansa({len(nansa_list)})"
os.makedirs(save_dir, exist_ok=True)
summary_df, events_df = Metadata_Quad_Sim_V3(nlist, b1list, b2list, d1list, d2list, nansa_list, tmax, skip, t_lag, save_dir, nbins, n_jobs=32)

end_time = time.perf_counter()
print(f"ansa={len(nansa_list)}_tmax={tmax}_Execution time: {end_time - start_time:.6f} seconds")
...
save_dir=f"/flash/DieckmannU/Abolfazl/Hump_({nlist[0]},{nlist[-1]})_b1({b1list[0]},{b1list[-1]})_b2({b2list[0]},{b2list[-1]})_d1({d1list[0]},{d1list[-1]})_d2({d2list[0]},{d2list[-1]})_skip({skip})_tmax({tmax})_tlag({t_lag})_nansa({len(nansa_list)})"
os.makedirs(save_dir, exist_ok=True)
summary_df, events_df = Metadata_Quad_Sim_V3(nlist, b1list, b2list, d1list, d2list, nansa_list, tmax, skip, t_lag, save_dir, nbins, n_jobs=5)
...
save_dir=f"/flash/DieckmannU/Abolfazl/Hump_({nlist[0]},{nlist[-1]})_b1({b1list[0]},{b1list[-1]})_b2({b2list[0]},{b2list[-1]})_d1({d1list[0]},{d1list[-1]})_d2({d2list[0]},{d2list[-1]})_skip({skip})_tmax({tmax})_tlag({t_lag})_nansa({len(nansa_list)})"
os.makedirs(save_dir, exist_ok=True)
summary_df, events_df = Metadata_Quad_Sim_V3(nlist, b1list, b2list, d1list, d2list, nansa_list, tmax, skip, t_lag, save_dir, nbins, n_jobs=2)
...
save_dir=f"/flash/DieckmannU/Abolfazl/Hump_({nlist[0]},{nlist[-1]})_b1({b1list[0]},{b1list[-1]})_b2({b2list[0]},{b2list[-1]})_d1({d1list[0]},{d1list[-1]})_d2({d2list[0]},{d2list[-1]})_skip({skip})_tmax({tmax})_tlag({t_lag})_nansa({len(nansa_list)})"
os.makedirs(save_dir, exist_ok=True)
summary_df, events_df = Metadata_Quad_Sim_V3(nlist, b1list, b2list, d1list, d2list, nansa_list, tmax, skip, t_lag, save_dir, nbins, n_jobs=25)
